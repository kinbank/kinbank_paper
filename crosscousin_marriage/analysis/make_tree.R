...
log = bt_read.log('crosscousin_marriage/results/bothXpermitted-dep-1.Log.txt')
log[log == "--"] = NA
...
d = read.table('crosscousin_marriage/data/bt_data/bothXpermitted.btdata', header = FALSE, sep = "\t")
rownames(d) = d[,1]
con_tree = keep.tip(phy = con_tree, tip = d$V1)

## create node probabilities
# values = colMeans(rec_nodes, na.rm = TRUE)
values = apply(rec_nodes, 2, mean, na.rm = T)
# nodes = str_match(names(values), "RecNode([0-9]+)")[,2]

# pairs nodes with nodes on tree
children = read.csv('crosscousin_marriage/data/tags.csv', skip = 1)
children_sets = children$X.1 %>% 
  str_trim() %>% 
  str_split(" ")
new_nodes = lapply(children_sets, function(x){ape::getMRCA(con_tree, tip = x)}) %>% unlist()

paired_nodes = data.frame(old_nodes = paste0("Rec", children$X, sep = ""), new_nodes = new_nodes)
result_nodes = data.frame(tree_nodes = str_match(names(values), "RecNode[0-9]+")[,1])
nodes = dplyr::left_join(result_nodes, paired_nodes, by = c("tree_nodes" = "old_nodes"))[,2]
...
nice_cols = c("#273253", "#ED5C4D", "#FBBE4B", "#57B5ED")
...
pdf(file = "crosscousin_marriage/ccm_plot.pdf",width = 6, height = 7)
...
legend(x = 0, y = 10, legend = c("No CM & No BM", "No CM & BM", "CM & No BM", 
                                "CM & BM"), pch = 19, col = nice_cols, bty = "n")
dev.off()

png(file = "crosscousin_marriage/ccm_plot.png")
plotTree(con_tree, setEnv = TRUE, offset = 0.5, fsize = 0.6, cex = 2)
nodelabels(node = as.numeric(rownames(node_prob_wide)), pie = node_prob_matrix, 
           piecol = nice_cols, cex = 0.5)
tiplabels(pch = 21, bg = tip_colours, cex = 0.8)
legend(x = 0, y = 15, legend = c("No CM & No BM", "No CM & BM", "CM & No BM", 
                                 "CM & BM"), pch = 19, col = nice_cols, bty = "n")
dev.off()
# nodelabels(cex = 0.7)

# Transitions ----
q_idx = str_detect(colnames(log), "^q")
transitions = log[,q_idx]
apply(transitions, 2, median)
